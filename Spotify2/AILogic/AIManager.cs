...
namespace Spotify2.AILogic
{
    internal class AIManager : IDisposable
    {
        ...
        {
            // Remove the sessionOptions creation from here since we create fresh ones in LoadModelAsync
        ...
        private async Task InitializeModel(string modelPath)
        {
            try
            {
                await LoadModelAsync(null, modelPath).ConfigureAwait(false); // Pass null since we create options inside
            }
            catch (Exception ex)
            {
                FileManager.LogError($"Error starting the model: {ex.Message}\n\nUnable to load model with DirectML.", true);
            }
            finally
            {
        ...
        private Task LoadModelAsync(SessionOptions sessionOptions, string modelPath)
        {
            try
            {
                // Get rid of existing model first
                _onnxModel?.Dispose();
                _onnxModel = null;
                
                FileManager.LogInfo("Loading model with DirectML");
                
                var freshSessionOptions = new SessionOptions
                {
                ...
                freshSessionOptions.AppendExecutionProvider_DML(0);
                
                _onnxModel = new InferenceSession(modelPath, freshSessionOptions);
                _outputNames = new List<string>(_onnxModel.OutputMetadata.Keys);

                FileManager.LogInfo("Successfully loaded model with DirectML");
                ValidateOnnxShape();
            }
            catch (OnnxRuntimeException ex)
            {
                ...
                if (ex.Message.Contains("DirectML execution provider is not enabled") ||
                    ex.Message.Contains("onnxruntime_providers_dml.dll"))
                {
                    message = "DirectML execution provider has not been found. Please ensure you have:\n" +
                            "1. Microsoft.ML.OnnxRuntime.DirectML NuGet package installed\n" +
                            "2. DirectX 12 compatible GPU\n" +
                            "3. Windows 10 version 1903 or later";
                    title = "DirectML Error";
                }
                else if (ex.Message.Contains("Provider DmlExecutionProvider has already been registered"))
                {
                    message = "DirectML provider registration conflict. The application will attempt to restart the model loading process.";
                    title = "DirectML Registration Error";
                }
                ...
                throw; // Re-throw to be caught by InitializeModel
            }
            catch (Exception ex)
            {
                ...
                if (Dictionary.toggleState["Show FPS"])
                {
                    ...
                    if (++fpsUpdateCounter >= fpsUpdateInterval)
                    {
                        ...
                        if (DetectedPlayerOverlay != null)
                        {
                            ...
                            {
                                if (DetectedPlayerOverlay.FpsLabel != null)
                                {
        ...
        private void UpdateConfidence(DetectedPlayerWindow detectedPlayerOverlay, double centerX, double centerY)
        {
            ...
            // Removed Application.Current.Dispatcher.InvokeAsync wrapper
            detectedPlayerOverlay.DetectedPlayerConfidence.Opacity = 1;
            detectedPlayerOverlay.DetectedPlayerConfidence.Content = $"{Math.Round((AIConf * 100), 2)}%";
