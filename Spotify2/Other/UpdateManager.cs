...
namespace Spotify2.Other
{
    internal class UpdateManager
    {
        ...
        public async Task CheckForUpdate(string currentVersion)
        {
            ...
            var (latestVersion, latestZipUrl) = await githubManager.GetLatestReleaseInfo("Goodknight2", "A.i.m.m.y-GK-Version");
        ...
        {
            // Download the newest release of Spotify to %temp%
            string envTempPath = Path.GetTempPath();
            string localZipPath = Path.Combine(envTempPath, "SpotifyUpdate.zip");
            ...
            string extractPath = Path.Combine(envTempPath, "SpotifyUpdate");
            await Task.Run(() => // Run extraction in a separate task
            {
                ZipFile.ExtractToDirectory(localZipPath, extractPath, true);
            });

            // Create a batch script to move the files and restart Spotify
