...
namespace Spotify2.Other
{
    internal class RequirementsManager
    {
        ...
        public static bool CheckForRequirements()
        {
            ...
            if (!IsCUDAInstalled())
            {
                MessageBox.Show("You don't have 12.6 CUDA Installed (or its improper install), you may not be able to use Spotify.", "Spotify");
            }

            if (!IsCUDNNInstalled())
            {
                MessageBox.Show("You don't have 9.3 CUDNN Installed (or its improper install), you may not be able to use Spotify.", "Spotify");
                return false;
            }
        ...
        public static bool IsCUDAInstalled()
        {
            try
            {
                ...
                if (dllExists)
                {
                    FileManager.LogInfo("Found all CUDA DLLS in Spotify Directory");
                    return true;
                }
        ...
        public static bool IsCUDNNInstalled()
        {
            try
            {
                ...
                if(File.Exists(Path.Combine(AppDomain.CurrentDomain.BaseDirectory +  "cudnn64_9.dll"))) {
                    FileManager.LogInfo("CUDNN 9.3 was found in Spotify directory.");
                    return true;
                }
